...
resource "aws_security_group" "ecs" {
  ...
  description = "ECS tasks: accept only from ALB"
  vpc_id      = aws_vpc.this.id
...
# Fallback HTTP listener used when no certificate is provided (e.g. initial bootstrap)
...
# Fargate Spot capacity provider mix: 70% Spot, 30% on-demand
# Spot cuts compute cost ~70% with automatic fallback to on-demand if capacity is unavailable
...
resource "aws_ecs_service" "this" {
  ...
  # Circuit breaker — auto-rollback on failed deployment
...
# Least-privilege task policy — only the permissions the app actually needs
